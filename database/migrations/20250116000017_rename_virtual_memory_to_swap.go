...
func (r *M20250116000017RenameVirtualMemoryToSwap) Up() error {
	...
	if facades.Schema().HasTable("server_virtual_memory") {
		// 重命名表
		facades.Orm().Query().Exec("ALTER TABLE server_virtual_memory RENAME TO server_swap")

		// 重命名字段
		facades.Orm().Query().Exec("ALTER TABLE server_swap CHANGE COLUMN virtual_memory_total swap_total BIGINT")
		facades.Orm().Query().Exec("ALTER TABLE server_swap CHANGE COLUMN virtual_memory_used swap_used BIGINT")
		facades.Orm().Query().Exec("ALTER TABLE server_swap CHANGE COLUMN virtual_memory_free swap_free BIGINT")

		// 重命名索引（如果存在）
		if facades.Schema().HasTable("server_swap") {
			// 先删除旧索引，再创建新索引（因为RENAME INDEX可能不支持）
			facades.Orm().Query().Exec("DROP INDEX IF EXISTS idx_server_virtual_memory_server_id ON server_swap")
			facades.Orm().Query().Exec("DROP INDEX IF EXISTS idx_server_virtual_memory_timestamp ON server_swap")
			facades.Orm().Query().Exec("CREATE INDEX idx_server_swap_server_id ON server_swap(server_id)")
			facades.Orm().Query().Exec("CREATE INDEX idx_server_swap_timestamp ON server_swap(timestamp)")
		}
	} else {
		// 如果表不存在，创建新表
